...
\begin{itemize}
  ...
  \item (S3) For closed $M \subseteq X$ of codimension $\le n$ and
    ...
    gives $\dim(W / M') \le \dim(X / M) \le n$. The factorisation
    $(B \circ S \circ A) \circ \iota_{M'} =
       B \circ (S \circ \iota_M) \circ \widehat{A}$,
    where $\widehat{A} : M' \to M$ is the corestriction of
    $A \circ \iota_{M'}$, then yields the bound by two applications of
    operator-norm submultiplicativity. No density-of-$\|\mathbb{K}\|$
    step.
