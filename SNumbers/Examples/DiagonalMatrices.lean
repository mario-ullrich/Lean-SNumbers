...
via two coordinate maps along the inclusion `Fin.castLE : Fin n → Fin m`:

* `projFin` keeps the first `n` coordinates;
* `padFin` extends a vector by zeros in the remaining coordinates.

Both are contractions, and `projFin ∘ padFin = id`. They let us build
finite-rank approximants and the factorisations behind the s-number bounds. -/
